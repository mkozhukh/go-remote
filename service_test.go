...
func getService(obj interface{}) *serviceProvider{
	t := newServiceProvider(newProvider())
	t.Add("", obj, nil)

	return t
}
func TestServiceCall(t *testing.T) {
	s := getService(StubCalck2{})
	thecall := callInfo{Name: "StubCalck2.Add", Args: []json.RawMessage{[]byte("2"), []byte("3")}}

	data, err := s.Value(&thecall)
	if err != nil {
		t.Error(err)
		return
	}
	...
	if data.(int) != 5 {
		t.Errorf("Invalid call result, 5 <> %d", data)
	}
}
